      ...
      if (mutation.addedNodes.length) {
        mutation.addedNodes.forEach((node) => {
          if (
            node.nodeType === 1 &&
            (node.matches(
              'button, .btn, input[type="submit"], input[type="button"], .catalog-search-btn, .show-button, .detail-button, .join-button, .login-btn, .create-startup-btn, .logout-btn, .nav-menu a'
            ) ||
              node.querySelector(
                'button, .btn, input[type="submit"], input[type="button"], .catalog-search-btn, .show-button, .detail-button, .join-button, .login-btn, .create-startup-btn, .logout-btn, .nav-menu a'
              ))
...
function initPositionAware() {
  const buttons = document.querySelectorAll(
    'button:not(.plyr__control):not(.plyr__control--back):not(.no-wave-effect), .btn:not(.plyr__control):not(.plyr__control--back), input[type="submit"]:not(.plyr__control):not(.plyr__control--back), input[type="button"]:not(.plyr__control):not(.plyr__control--back), .catalog-search-btn:not(.plyr__control):not(.plyr__control--back), .show-button:not(.plyr__control):not(.plyr__control--back), .detail-button:not(.plyr-control):not(.plyr-control--back), .join-button:not(.plyr__control):not(.plyr__control--back), .login-btn:not(.plyr__control):not(.plyr__control--back), .create-startup-btn:not(.plyr__control):not(.plyr__control--back), .logout-btn:not(.plyr__control):not(.plyr__control--back), .nav-menu a:not(.plyr__control):not(.plyr__control--back)'
  )
  buttons.forEach((button) => {
    ...
    waveSpan.style.transition =
      'width 0.4s ease-in-out, height 0.4s ease-in-out, opacity 0.4s ease-in-out, transform 0.4s ease-in-out'
...
function handleTouchStart(e) {
  ...
  if (!waveSpan) {
    ...
    waveSpan.style.zIndex = '-1'
    waveSpan.style.transition =
      'width 0.6s ease-out, height 0.6s ease-out, opacity 0.6s ease-out, transform 0.6s ease-out'
    button.appendChild(waveSpan)
  }
  waveSpan.style.width = `${diameter}px`
  waveSpan.style.height = `${diameter}px`
